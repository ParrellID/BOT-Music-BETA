...
client.on("message", async message => {
  ...
  if (message.content.match(prefixMention)) {
    ...
    let embed = new MessageEmbed()
        ...
        .setDescription(`
		Hello **${message.author.username}** 👋, I Was Made by <@${developerID}> 
        ...
        :question: Still Need Help? [Click Here](https://wa.me/6285807231447) to Talk About BOTz
        `)
...
let ad = ads.ad[Math.floor((Math.random() * ads.ad.length))];
  if(cooldown.has(message.author.id)){
    ...
    return message.channel.send(`[❗] **${message.author.username}** Penggunaan Fitur diberi Cooldown, Harap Bersabar & Tidak Spam bila menggunakan Fitur BOTz!! \n\n ${ad}`)
  }
...
cooldown.delete(message.author.id)}, cdseconds * 1000)
  ...
  if (command.premium) {
    ...
    if (!guild) {
      return message.channel.send(`INFO-BOT: \n\nAnda Dapat menggunakan Fitur ini hanya di Server Premium. \n **Ingin Menjadikan Server Anda Premium?** Donate US!: https://saweria.co/farrellathar`)
    }
 ...
 if (message.channel.id == chan) {
    ...
    message.channel.startTyping();
     ...
     if(isNaN(message.content)) {
       message.delete();
                return message.author.send(`[❗] Sertakan Nomor untuk Melakukan Lebih Lanjut!`)
...
require('http').createServer((req, res) => res.end(`
 ...
 |• Alive: 24/7                            |
 |-----------------------------------------|
 |• Author: Parreelll#7172                  |
 |-----------------------------------------|
 |• Donate: https://saweria.co/farrellathar  |
 |-----------------------------------------|
 |• Github: https://github.com/FarrellID |
 |-----------------------------------------|
